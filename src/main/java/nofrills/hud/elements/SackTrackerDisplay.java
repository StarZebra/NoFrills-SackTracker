...
public class SackTrackerDisplay extends SimpleTextElement {
    public final Feature instance = new Feature("sackTrackerElement");
    ...
    private final Identifier identifier = Identifier.of("sacktracker", "sack-tracker-element");
    private MutableText lastText = ShardTracker.displayNone;



    public SackTrackerDisplay(double x, double y) {
        super(SackTracker.displayNone);
        ...
        this.options.setTitle(Text.of("Sack Tracker Element"));
    }

    public SackTrackerDisplay() {
        this(HudManager.getDefaultX(), HudManager.getDefaultY());
    }

    @Override
    public void draw(OwoUIDrawContext context, int mouseX, int mouseY, float partialTicks, float delta) {
        boolean trackerActive = SackTracker.instance.isActive();
        MutableText text = trackerActive ? SackTracker.display : SackTracker.displayNone;
        if (HudManager.isEditingHud()) {
            ...
            if (hideIfNone.value()) {
                if (!trackerActive || text.equals(SackTracker.displayNone)) {
                    return;
                }
            }
        }
